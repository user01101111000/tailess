...
/** Default filename for the generated type-augmentation. */
export const DEFAULT_DTS_NAME = "tailess-env.d.ts";

/**
 * Render the `.d.ts` that teaches the top-level `"tailess"` helpers your config's
 * custom keys. Emits a `Register` module augmentation with the config's `screens`
 * and `states` keys, so `import { ss } from "tailess"` autocompletes and
 * type-checks them — no hand-written `declare module` needed.
 */
export function renderConfigDts(config: Pick<TailessConfig, "screens" | "states">): string {
  const keysType = (obj?: Record<string, string>): string => {
  ...
  return `// Auto-generated by tailess. Do not edit.
// Regenerated from your tailess config whenever the PostCSS plugin runs.
import "tailess";

declare module "tailess" {
  interface Register {
    config: { screens: ${keysType(config.screens)}; states: ${keysType(config.states)} };
  }
}
`;
}
...
/**
 * Write the generated type-augmentation to `outPath` (default `tailess-env.d.ts`
 * in the current working directory). Skips the write when the content is already
 * up to date, so it never triggers a redundant rebuild in watch mode.
 */
export async function writeConfigDts(
  config: Pick<TailessConfig, "screens" | "states">,
  outPath?: string,
): Promise<"written" | "unchanged"> {
  const target = outPath ? resolve(outPath) : resolve(process.cwd(), DEFAULT_DTS_NAME);
  const content = renderConfigDts(config);
